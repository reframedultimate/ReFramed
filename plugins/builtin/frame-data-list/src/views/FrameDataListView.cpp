...
void FrameDataListView::repopulateGameInfoTable()
{
    ...
    for (int i = 0; i != session->fighterCount(); ++i)
    {
        const rfcommon::String* fighterName = session->mappingInfo().fighterID.map(session->fighterID(i));
        ui_->tableWidget_gameInfo->setItem(6+i, 0, new QTableWidgetItem(session->name(i).cStr()));
